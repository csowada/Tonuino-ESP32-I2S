...
void PN532::begin() 
{
    ...
    #elif USE_HARDWARE_I2C
    {
        ...
        /** dump serial buffer */
        if (HsuClass::Available()) {
            Utils::Print("Dump serial buffer: ");
        }
        while (HsuClass::Available()) {
            uint8_t ret = HsuClass::Read();
            Utils::PrintHex8(ret);
        }
...
{ 
    ...
    #elif USE_HARDWARE_I2C
    {
        ...
        for (byte i=0; i<len; i++) 
        {
            buff[i] = HsuClass::Read();
            // Utils::Print("ReadPacket(): read2 ");
            // Utils::PrintHex8(buff[i], LF);
        }
        return true;
    }
    #endif
